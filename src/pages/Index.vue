...
export default defineComponent({
  ...
  setup() {
    ...
    const DraggedColumn = (data) => {
      let ordination = columns.value.map((col) => col.name);

      ordination = changePosition(ordination, data.old_index, data.new_index);

      const columnsNew = [];
      ordination.forEach((order) => {
        const col = columns.value.find((col) => col.name === order);
        columnsNew.push(col);
      });

      console.log(ordination);

      localStorage.setItem("@@grid", JSON.stringify(columnsNew));
    };
